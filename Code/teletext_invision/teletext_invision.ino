...
long IR_ZERO= eepromReadLong(4);
long IR_ONE=  eepromReadLong(8);
long IR_TWO=  eepromReadLong(12);
long IR_THREE=eepromReadLong(16);
long IR_FOUR= eepromReadLong(20);
long IR_FIVE= eepromReadLong(24);
long IR_SIX=  eepromReadLong(28);
long IR_SEVEN=eepromReadLong(32);
long IR_EIGHT=eepromReadLong(36);
long IR_NINE= eepromReadLong(40);

long IR_UP=   eepromReadLong(44);
long IR_DOWN= eepromReadLong(56);
long IR_ENTER=eepromReadLong(60);
...
void setup() {
  ...
  {
    Serial.println("SAA5244 absent or faulty!");
    while(true)
    {
  ...
  while(!teletext.callInit()) { // wait to get 8/30
    ...
    {
      Serial.println("No 8/30, calling P100");
...
void setupMenu()
{
  Serial.println("Setup Mode - use the onboard buttons");
  ...
  while(true)
  {
    ...
    if(digitalRead(6)==LOW)
    {
      ...
      {
        Serial.println("Pairing new remote");
        pairIR();
        delay(2000);
        resetFunc();
      }
...
void fillscreen()
{
  ...
  char string[]="NMS (c) V1.0 May 2020";
  for(int i =0; i < strlen(string); i++ ) {
    char c = string[i];
    teletext.writeByte(0x0b,c);
  }
...
void pairIR()
{
  ...
  for(int i=0; i<=16; i++)
  {
    ...
    {
      eepromWriteLong(results.value,((i+1)*4));
    }
  }
